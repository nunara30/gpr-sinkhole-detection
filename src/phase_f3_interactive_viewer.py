"""
Phase F-3 (v2) - Interactive Viewer — YOLO + EfficientNet 앙상블
.dt 파일 목록을 좌측 패널에 표시하고, 선택 시 앙상블 추론 결과를 우측에 실시간 표시.
  - YOLO (phase_g/run_n20): sinkhole / tunnel  → bbox
  - EfficientNet (phase_k): pipe / rebar       → 이미지 좌측 컬러 바
...
from phase_n_ensemble import EnsemblePipeline, preprocess_dt as ensemble_preprocess
...
def preprocess_dt(dt_path: Path) -> np.ndarray | None:
    return ensemble_preprocess(dt_path)
...
class GPRViewer:
    def __init__(self, root: tk.Tk, file_list: list, pipeline: EnsemblePipeline):
        self.root       = root
        self.file_list  = file_list
        self.pipeline   = pipeline
        self.cur_idx    = 0
        self._cache     = {}          # dt_path → img_bgr (전처리 캐시)
        self._busy      = False

        root.title("GPR Interactive Viewer  — Phase N (YOLO + EfficientNet Ensemble)")
    ...
    def _build_ui(self):
        ...
        # conf 슬라이더 (YOLO / EfficientNet 각각)
        ctrl = tk.Frame(right, bg="#121212")
        ctrl.pack(fill=tk.X, padx=10, pady=6)

        def _make_slider(parent, label, default, var_attr):
            tk.Label(parent, text=label, bg="#121212",
                     fg="#aaaaaa", font=("Consolas", 9)).pack(side=tk.LEFT)
            var = tk.DoubleVar(value=default)
            setattr(self, var_attr, var)
            tk.Scale(
                parent, from_=0.01, to=0.99, resolution=0.01,
                orient=tk.HORIZONTAL, variable=var, length=160,
                bg="#121212", fg="#cccccc", troughcolor="#333333",
                highlightthickness=0,
                command=lambda _: self._reload(),
            ).pack(side=tk.LEFT, padx=4)
            tk.Label(parent, textvariable=var, bg="#121212", fg="#cccccc",
                     font=("Consolas", 9), width=4).pack(side=tk.LEFT, padx=(0, 12))

        _make_slider(ctrl, "YOLO conf:",  0.25, "yolo_conf_var")
        _make_slider(ctrl, "Cls conf:",   0.60, "cls_conf_var")
    ...
    def _load_thread(self, idx, cls_name, dt_path):
        ...
        if key not in self._cache:
            self._cache[key] = preprocess_dt(dt_path)
        ...
        # 앙상블 추론 (conf 슬라이더 값 반영)
        self.pipeline.yolo_conf = self.yolo_conf_var.get()
        self.pipeline.cls_conf  = self.cls_conf_var.get()
        result = self.pipeline.run(dt_path, img_bgr=img_bgr)

        # True 클래스 라벨 오버레이
        vis = result["vis_bgr"].copy()
        cv2.putText(vis, f"True: {cls_name}", (6, 22),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 0), 2, cv2.LINE_AA)

        self.root.after(0, lambda: self._update_display(
            vis, cls_name, dt_path, result, idx))
        self._busy = False

    def _update_display(self, vis, cls_name, dt_path, result, idx):
        rgb = cv2.cvtColor(vis, cv2.COLOR_BGR2RGB)
        pil = Image.fromarray(rgb).resize((DISPLAY_W, DISPLAY_H), Image.LANCZOS)
        self._tk_img = ImageTk.PhotoImage(pil)
        self.img_label.config(image=self._tk_img)

        det_cls  = [d["cls"] for d in result["detections"]]
        cls_pred = [c["cls"] for c in result["classifications"]]
        pred_all = det_cls + cls_pred
        tp = cls_name in pred_all
        status = (f"[{idx+1}/{len(self.file_list)}]  True: {cls_name}  |  "
                  f"YOLO: {det_cls or ['–']}  |  Cls: {cls_pred or ['–']}  |  "
    ...
    def _reload(self):
        """conf 슬라이더 변경 시 현재 파일 재추론 (전처리 캐시 재활용)."""
        if not self._busy:
...
def main():
    print("\n" + "=" * 60)
    print("  GPR Interactive Viewer — Phase N Ensemble")
    print("  YOLO (sinkhole/tunnel) + EfficientNet (pipe/rebar)")
    print("=" * 60)

    print("앙상블 파이프라인 로드 중...")
    pipeline = EnsemblePipeline()

    print("테스트 파일 수집 중...")
    trained_stems = _load_trained_stems()
    file_list = collect_files(trained_stems)
    print(f"  {len(file_list)}개 파일")
    for cls in CATEGORIES:
    ...
    app = GPRViewer(root, file_list, pipeline)
    root.mainloop()
